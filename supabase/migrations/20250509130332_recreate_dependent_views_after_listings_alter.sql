-- Migration: Recreate views dependent on the listings table

-- View: archived_listings_details
...
SELECT
    ...
    l.description,
    l.min_price,
    l.photos, -- Will now reference the TEXT[] column
    l.end_time,
    l.created_at,
    l.status,
    l.upper_cap,
...
FROM
    public.listings l
LEFT JOIN
    auth.users seller ON l.seller_id = seller.id
LEFT JOIN
    public.bids winning_bid ON l.winning_bid_id = winning_bid.id
LEFT JOIN
    auth.users winner ON l.winning_bidder_id = winner.id
WHERE
    l.status = ANY (ARRAY['closed'::text, 'cancelled'::text]);
...
-- View: listings_with_highest_bid
...
-- View: listings_with_seller_email
...
-- Add any other views you dropped in Migration File 1 here, along with their GRANTS