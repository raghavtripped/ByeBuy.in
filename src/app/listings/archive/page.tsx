...
const parsePhotosJson = (
  ...
  if (Array.isArray(photosInput)) {
    ...
    console.warn(
      'Photos input is an array but not uniformly strings:',
      photosInput
    );
    return null;
  }
  ...
  if (typeof photosInput === 'string') {
    try {
      ...
      console.warn(
        'Parsed photos JSON string is not an array of strings:',
        parsed
      );
      return null;
    } catch (error) {
...
export default function ArchivedListingsPage() {
  ...
  const fetchArchivedItemDetails = useCallback(
    async (itemId: string): Promise<ArchivedListingDisplay | null> => {
      ...
      if (itemData.status === 'closed' || itemData.status === 'cancelled') {
        return {
          ...itemData,
          photos: parsePhotosJson(itemData.photos),
  ...
  const handleRealtimeUpdate = useCallback(
    async (
      payload: RealtimePostgresChangesPayload<ListingTableRecord>
    ) => {
      const newRecord =
