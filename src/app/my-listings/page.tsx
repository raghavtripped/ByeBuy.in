...
export default function MyListingsPage() {
  ...
  /* --------------------------- fetch listings ------------------------ */
  const fetchUserDataAndListings = useCallback(async (u: User) => {
    setLoading(true);
    setError(null);
    setDeleteError(null);
    ...
    try {
      ...
      const typed = (data ?? []).map((l) => ({
        ...l,
        photos: parsePhotosJson(l.photos as string | string[] | null),
          ...
          case 'INSERT':
            if (
              ...
              const fresh: SellerListing = {
                ...
                photos: parsePhotosJson(newRec.photos),
            ...
            if (newRec) {
              ...
              if (idx !== -1) {
                ...
                next[idx] = {
                  ...
                  ...newRec,
                  photos:
                    newRec.photos !== undefined
                      ? parsePhotosJson(newRec.photos)
                      : cur.photos,
                  ...
                  end_time:
                    newRec.end_time !== undefined ? newRec.end_time : cur.end_time,
  ...
  const handleFinalizeAuction = async (id: string) => {
    ...
    try {
      const { data, error } = await supabase.rpc('finalize_auction_outcome', {
  ...
  const handleDeleteListing = async (id: string, photos: string[] | null) => {
    ...
    try {
      ...
      if ((count ?? 0) > 0)
        throw new Error('Cannot delete: bids already placed.');
      ...
      {filteredListings.length === 0 ? (
        ...
        <ul className="space-y-6">
          {filteredListings.map((l) => {
            ...
            const thumb =
              l.photos && l.photos.length ? l.photos[0] : null;
