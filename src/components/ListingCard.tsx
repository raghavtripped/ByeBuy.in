...
export default function ListingCard({ listing, currentUser }: ListingCardProps) {
  ...
  return (
    ...
    <li className="group relative flex flex-col bg-white dark:bg-bye-dark-bg-secondary rounded-lg shadow border border-gray-200 dark:border-bye-dark-border-primary overflow-hidden transition-all duration-200 hover:shadow-lg hover:-translate-y-1">
      ...
      <Link href={`/listings/${listing.id}`} className="flex flex-col flex-grow">
        {/* Updated image placeholder background for dark mode */}
        <div className="aspect-video w-full bg-gray-100 dark:bg-bye-dark-bg-hover overflow-hidden relative rounded-t-lg">
          {thumbnailUrl ? (
            ...
            // Updated placeholder icon color for dark mode
        ...
        <div className="p-4 flex flex-col flex-grow">
          ...
          {/* Updated divider border color and text colors within this section */}
          <div className="mt-auto pt-2 space-y-1 text-sm border-t border-gray-100 dark:border-bye-dark-border-primary opacity-80 dark:opacity-100"> {/* Slightly adjusted opacity for light mode divider */}
            <p className="text-gray-600 dark:text-bye-dark-text-secondary">
              Min Bid:{' '}
              {/* Indigo accent for price, ensure dark:text-indigo-400 contrasts well */}
            ...
            {listing.status === 'active' && listing.current_highest_bid && listing.current_highest_bid > 0 ? (
              <p className="text-gray-600 dark:text-bye-dark-text-secondary">
                Top Bid:{' '}
                {/* Green accent for top bid, ensure dark:text-green-400 contrasts well */}
                <span className="font-semibold text-green-700 dark:text-green-400"> 
                  {formatCurrency(listing.current_highest_bid)}
                </span>
              </p>
            ) : listing.status === 'active' ? (
              // Updated "No bids yet" text color for dark mode
            ...
            {isEffectivelyEnded && listing.status !== 'active' && (
              // Status text colors, ensure they contrast well
              <p className={`text-xs font-medium mt-0.5 ${
                listing.status === 'closed' ? 'text-green-600 dark:text-green-400' 
                : listing.status === 'cancelled' ? 'text-yellow-600 dark:text-yellow-500' // Adjusted yellow for better dark contrast
