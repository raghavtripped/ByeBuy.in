...
export default function ListingDetails() {
  ...
  useEffect(() => {
    ...
    if (!id || !uuidRegex.test(id)) { /* ... handle invalid id ... */ setError("Listing not found."); setLoading(false); return; }
    ...
    // Load initial data
    const load = async () => { /* ... same initial load function ... */
        try {
    ...
    bidsChannel.on(
      ...
      .subscribe((status, err) => { // Subscribe callback remains the same
  ...
  // placeBid function remains the same (with bid status message logic)
  const placeBid = async () => { /* ... placeBid function ... */
  ...
  // Render Logic remains the same (with currency formatting)
