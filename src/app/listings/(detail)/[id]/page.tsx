...
// Type Definitions (Matching previous working state)
type Listing = {
    id: string;
    title: string;
    description: string;
    ...
    seller_id?: string; // Keep if selected in view
};
...
export default function ListingDetails() {
  ...
  // State variables (Matching previous working state)
  ...
  // useEffect (Matching previous working state with correct RT calls)
  useEffect(() => {
    ...
    const load = async () => {
        try {
            const { data: lData, error: lError } = await supabase
              ...
              .select(` id, title, description, min_price, photos, end_time, upper_cap, rules, seller_email, seller_id `) // Ensure all needed fields selected
              .eq('id', id)
              .maybeSingle();
            ...
            if (lData) {
                ...
                setError("Listing not found."); setBids([]);
            }
        } catch (err) {
  ...
  // --- placeBid Function with ONLY Upper Cap Check Added ---
  const placeBid = async () => {
    ...
    if (listing.end_time && isPast(listing.end_time)) {
        ...
        setBidStatusMessage(`⚠️ ${msg}`); return;
    }

    // 2. Validate bid amount format
    const amt = parseFloat(price);
     if (isNaN(amt)) {
         setBidStatusMessage(`⚠️ Please enter a valid bid amount.`); return;
     }

    // 3. Check against minimum required bid
    const currentHighestBid = bids[0]?.bid_price ?? 0;
    const minBidRequired = Math.max(listing.min_price, currentHighestBid);
    if (amt <= minBidRequired) {
        const msg = `Bid must be greater than ${formatCurrency(minBidRequired)}`;
        setBidStatusMessage(`⚠️ ${msg}`); return;
    }

        // --- 4. REFINED: Check against Upper Cap / Buy Now Price ---
        if (listing.upper_cap && listing.upper_cap > 0) { // Check if upper_cap exists and is positive
          if (amt > listing.upper_cap) { // Bid EXCEEDS Buy Now price
               const msg = `Bid cannot exceed the Buy Now price of ${formatCurrency(listing.upper_cap)}.`;
               setBidStatusMessage(`⚠️ ${msg}`);
               return; // Stop processing the bid
          }
          if (amt === listing.upper_cap) { // Bid EQUALS Buy Now price
               const msg = `Bidding ${formatCurrency(listing.upper_cap)} triggers the Buy Now option (feature coming soon!). Please use the Buy Now button when available or place a lower bid.`;
               setBidStatusMessage(`⚠️ ${msg}`);
               return; // Stop processing the bid
          }
          // If amt < listing.upper_cap, the bid is allowed (assuming it passed minBidRequired check)
      }
      // --- End REFINED Check ---
    ...
    // 5. Proceed with inserting the bid (Existing Logic)
    console.log(`DEBUG: Placing bid: User ${user.id}, Item ${id}, Amount ${amt}`);
    ...
    // Clear message after delay (Existing Logic)
    setTimeout(() => { setBidStatusMessage(null); }, 3000);
  };
  // --- End placeBid Function ---


  // Render Logic (Matching previous working state)
  ...
  return (
    // --- Full JSX structure (Identical to previous working version) ---
