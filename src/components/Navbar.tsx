...
// ---------- Helpers ----------
/** Return the previous value of any state/prop. */
...
export default function Navbar() {
  ...
  const prevUser = usePrevious(user); // NEW: track previous user
  ...
  useEffect(() => {
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange((_event, session) => {
      ...
      console.log(
        LOG_PREFIX,
        'Effect 2 – onAuthStateChange FIRED. Event:',
        _event,
        'User ID:',
        session?.user?.id
      );
  ...
  /* ── Effect 2.1: close menu only on *actual* logout ─ */
  useEffect(() => {
    console.log(
      LOG_PREFIX,
      'Effect 2.1 (Close on SignOut) RUNNING. Current User:',
      user?.id,
      'Prev User:',
      prevUser?.id,
      'Menu Open:',
    ...
    // Close the menu only if a user existed before, is now null, *and* the menu is open
    if (prevUser && !user && isMobileMenuOpen) {
      console.log(LOG_PREFIX, 'Effect 2.1 – User JUST signed out & menu open, closing menu.');
      setIsMobileMenuOpen(false);
    }
  }, [user, prevUser, isMobileMenuOpen]);

  /* ── Effect 3: close menu on real route change ─── */
  useEffect(() => {
    console.log(
      LOG_PREFIX,
      'Effect 3 (Route Change) RUNNING. Pathname:',
      pathname,
      'Prev Pathname:',
      prevPathname,
      'Menu Open:',
    ...
    if (prevPathname !== undefined && pathname !== prevPathname && isMobileMenuOpen) {
      console.log(
        LOG_PREFIX,
        'Effect 3 – Pathname ACTUALLY changed from',
        prevPathname,
        'to',
        pathname,
        'closing mobile menu.'
  ...
  }, [isMobileMenuOpen]); // dependency intentionally empty
