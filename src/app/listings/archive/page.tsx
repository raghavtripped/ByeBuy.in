...
type ArchivedListingDisplay = {
  ...
  title: string;
  min_price: number;
  photos: string[] | null; // Parsed: array or null
...
type ArchivedViewItem = {
  ...
  title: string;
  min_price: number;
  photos: string | string[] | null; // Raw from view: can be string or array before parsing
  end_time: string | null;
  created_at?: string;
  status: string;
  seller_email?: string | null;
...
type ListingTableRecord = Partial<{
  ...
  title: string;
  min_price: number;
  photos: string | string[] | null; // Raw from DB: can be string or array before parsing
  end_time: string | null;
  created_at: string;
...
export default function ArchivedListingsPage() {
  ...
  const fetchArchivedItemDetails = useCallback(async (itemId: string): Promise<ArchivedListingDisplay | null> => {
    ...
    if (itemData.status === 'closed' || itemData.status === 'cancelled') {
        return {
            ...itemData,
            photos: parsePhotosJson(itemData.photos), // Use parser
      ...
      try {
        ...
        const validArchived = rawDataFromView
            ...
            .map((item): ArchivedListingDisplay => ({
                ...item,
                photos: parsePhotosJson(item.photos), // Use parser
            }));
  ...
  }, [handleRealtimeUpdate, sortArchived, fetchArchivedItemDetails]); // fetchArchivedItemDetails added
