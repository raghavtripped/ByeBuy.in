...
const CATEGORIES_FOR_FORM = [
  ...
  'Apparel & Accessories',
  'Sports & Hobby Gear',
  // Consider adding 'Other' back if it was intended
];
const MAX_PHOTOS = 5;

interface UploadResult {
  status: 'fulfilled' | 'rejected';
  value?: string; // URL of the uploaded photo
  reason?: unknown;
  filename: string;
}
...
export default function NewListingPage() {
  ...
  const [photos, setPhotos] = useState<File[]>([]); // Stores File objects for upload
  const [photoPreviews, setPhotoPreviews] = useState<string[]>([]); // Stores Object URLs for previews
  ...
  // Effect to create and revoke object URLs for photo previews
  useEffect(() => {
    const objectUrls = photos.map((file) => URL.createObjectURL(file));
    setPhotoPreviews(objectUrls);

    // Cleanup function to revoke object URLs when component unmounts or photos change
    return () => {
  ...
  const handleRemovePhoto = (idxToRemove: number) => {
    setPhotos((prevPhotos) => prevPhotos.filter((_, index) => index !== idxToRemove));
    // Photo previews will update automatically due to the useEffect dependency on `photos`
  };

  const handleCategorySelect = (category: string) => {
    setSelectedCategory(category);
    setSubmitMessage(null); // Clear previous messages when category changes
  };

  /* ---------- Submit ----------------------------------------------------- */
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => { // Corrected type to FormEvent
    e.preventDefault();
    setSubmitMessage(null);
    ...
    if (upperCapFloat !== null) {
      if (isNaN(upperCapFloat)) {
        setSubmitMessage({ type: 'error', text: 'Buy Now price must be a valid number if provided.' });
        return;
      }
    ...
    if (photos.length > 0) {
      const uploadPromises: Promise<UploadResult>[] = photos.map((photoFile) => {
        ...
        const safeTitle = title.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 30) || 'listing';
        // Ensure user.id is available (it should be due to validation above)
        ...
        return supabase.storage
          ...
          .catch((reason) => {
            console.error('Upload promise rejected:', reason);
            return { status: 'rejected' as const, reason, filename: photoFile.name };
          });
      });

      const results = await Promise.allSettled(uploadPromises); // Use allSettled
      ...
      results.forEach(result => {
        if (result.status === 'fulfilled' && result.value.status === 'fulfilled' && result.value.value) {
          ...
          const msg = resVal.reason instanceof Error ? resVal.reason.message : String(resVal.reason);
          uploadErrors.push(`'${resVal.filename}': ${msg}`);
        } else if (result.status === 'rejected') {
          // This case should ideally be caught by the .catch in the promise
          uploadErrors.push(`A photo upload promise was rejected entirely: ${result.reason}`);
        }
      });


      if (photos.length > 0 && uploadedPhotoUrls.length === 0) { // Check if original photos existed but none uploaded
        setSubmitMessage({
          type: 'error',
          text: `Photo upload failed for all images. Errors: ${uploadErrors.join('; ')}`,
        });
    ...
    try {
      const listingPayload = {
        ...
        seller_id: user.id, // user is guaranteed to be non-null here
        photos: uploadedPhotoUrls.length > 0 ? uploadedPhotoUrls : null, // CORRECTED KEY
        upper_cap: upperCapFloat,
        rules: rules.trim() || null,
        status: 'active' as const,
        tags: selectedCategory ? [selectedCategory] : null, // CORRECTED KEY
      };
      
      console.log("Attempting to insert payload:", JSON.stringify(listingPayload, null, 2));
      ...
      if (insertError) {
        ...
        throw insertError; // Throw to be caught by the outer catch
      }

      let successMessage = 'Listing created successfully!';
      if (uploadErrors.length > 0) {
        successMessage += ` However, ${uploadErrors.length} photo(s) failed to upload: ${uploadErrors.join('; ')}`;
      }
      setSubmitMessage({ type: 'success', text: successMessage + ' Redirecting…' });
      ...
      setRules('');
      setPhotos([]);
      // photoPreviews will be cleared by the useEffect on `photos`
      setSelectedCategory('');
      const photoUploadInput = document.getElementById('photo-upload') as HTMLInputElement | null;
      if (photoUploadInput) photoUploadInput.value = ''; // Clear file input

      setTimeout(() => router.push('/listings'), uploadErrors.length > 0 ? 4000 : 2000);
    } catch (err) {
      console.error('Listing insertion or final processing failed:', err);
      let userFriendlyMessage = 'Database error occurred while creating the listing.';
      if (err instanceof Error) {
        // Check for specific Supabase error details if available
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const supabaseError = err as any; // Type assertion to access potential Supabase error properties
        if (supabaseError.message) {
            userFriendlyMessage = `Error: ${supabaseError.message}`;
        }
        if (supabaseError.details) {
            userFriendlyMessage += ` Details: ${supabaseError.details}`;
        }
        if (supabaseError.hint) {
            userFriendlyMessage += ` Hint: ${supabaseError.hint}`;
        }
      }
      if (uploadErrors.length > 0) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <form
        ...
        noValidate
        className="bg-white dark:bg-bye-dark-bg-secondary
                   border border-gray-200 dark:border-bye-dark-border-primary
                   rounded-lg shadow-md p-6 sm:p-8 space-y-6"
      >
        {/* Submission feedback */}
        {submitMessage && (
          <div
            role="alert"
            className={`p-4 rounded-md border text-sm flex items-start gap-3 ${ // items-start for icon alignment
          ...
          >
            <svg
              className="w-5 h-5 flex-shrink-0 mt-0.5" // Icon styling
            ...
            <span className="flex-grow">{submitMessage.text}</span> {/* flex-grow for text wrapping */}
          </div>
        )}
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border
                       border-gray-300 dark:border-bye-dark-border-primary
                       rounded-md shadow-sm text-sm
                       focus:ring-indigo-500 focus:border-indigo-500
                       dark:focus:ring-indigo-400 dark:focus:border-indigo-400
                       dark:bg-bye-dark-bg-hover dark:text-bye-dark-text-primary
                       dark:placeholder-bye-dark-text-secondary"
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border
                       ...
                       dark:placeholder-bye-dark-text-secondary"
          />
          <p className="mt-1 text-xs text-gray-500 dark:text-bye-dark-text-secondary">
            Select date & time when the auction should automatically end.
          </p>
        </div>
          ...
          <div className="flex flex-wrap gap-2 mt-1.5">
            {CATEGORIES_FOR_FORM.map(cat => (
              <button
                ...
                className={`px-4 py-2 text-xs sm:text-sm font-medium rounded-full border
                            transition-all duration-150 ease-in-out
                            focus:outline-none focus:ring-2 focus:ring-indigo-500
                            dark:focus:ring-indigo-400
                            focus:ring-offset-2 dark:focus:ring-offset-bye-dark-bg-secondary
                            ${
        ...
        <div>
          ...
          <label
            ...
            className="cursor-pointer inline-flex items-center px-4 py-2 border
                       ...
                       text-gray-700 dark:text-bye-dark-text-primary
                       bg-white dark:bg-bye-dark-bg-hover
                       hover:bg-gray-50 dark:hover:bg-opacity-75
                       focus-within:outline-none focus-within:ring-2 focus-within:ring-indigo-500
                       dark:focus-within:ring-indigo-400
                       dark:focus-within:ring-offset-bye-dark-bg-secondary" // Ensure focus offset matches dark theme
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 16 16"
              fill="currentColor"
              className="w-4 h-4 mr-2 text-gray-500 dark:text-bye-dark-text-secondary"
            >
            ...
            <span>
              {photos.length > 0
                ? `Add More / Change Photos (${photos.length}/${MAX_PHOTOS})`
                : 'Choose Photos'}
            </span>
            <input
              id="photo-upload"
              ...
              multiple
              required={photos.length === 0} // Only required if no photos yet
                ...
                <div key={url} className="relative group aspect-square">
                  ...
                  <button
                    ...
                    className="absolute top-1 right-1 p-0.5 bg-red-600 text-white rounded-full
                               ...
                               dark:focus:ring-offset-bye-dark-bg-secondary" // Ensure focus offset matches dark theme
          ...
          <p className="mt-1 text-xs text-gray-500 dark:text-bye-dark-text-secondary">
            First image will be the cover photo. PNG, JPG, WEBP accepted.
          </p>
        </div>
        ...
        <div className="pt-4 border-t border-gray-200 dark:border-bye-dark-border-primary">
          <button
            ...
            className="w-full sm:w-auto inline-flex justify-center items-center px-6 py-2.5
