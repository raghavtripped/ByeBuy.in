...
export default function NewListingPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPhotos(prev => {
      ...
      if (combined.length > MAX_PHOTOS) {
        setSubmitMessage({
          type: 'error',
          text: `You can upload a maximum of ${MAX_PHOTOS} photos.`,
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(minPriceFloat) || minPriceFloat < 0) {
      setSubmitMessage({
        type: 'error',
        text: 'Minimum Bid Price must be a valid non-negative number.',
      });
      return;
    }
    if (upperCapFloat !== null) {
      ...
      if (upperCapFloat <= minPriceFloat) {
        setSubmitMessage({
          type: 'error',
          text: 'Buy Now price must exceed the Minimum Bid Price.',
        });
        return;
      }
    }
    if (new Date(endTime) <= new Date()) {
      setSubmitMessage({
        type: 'error',
        text: 'Auction End Time must be set to a future date/time.',
      });
      return;
    }
    /* -- End validation --------------------------------------------------- */
    ...
    if (photos.length) {
      const uploadPromises: Promise<UploadResult>[] = photos.map(photo => {
        ...
        const path      = `${user.id}/${safeTitle}_${Date.now()}_${Math.random()
          .toString(36)
          .substring(2, 7)}.${ext}`;
      ...
      if (uploadedPhotoUrls.length === 0) {
        setSubmitMessage({
          type: 'error',
          text: `Photo upload failed. Errors: ${uploadErrors.join('; ')}`,
        });
        ...
        <div>
          ...
          <label
            ...
            className="cursor-pointer inline-flex items-center px-4 py-2 border
                       ...
                       dark:focus-within:ring-offset-bye-dark-bg-secondary"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 16 16"
