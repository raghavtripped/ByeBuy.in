-- supabase/migrations/20250524095325_final_jsonb_column_swap_and_view_update.sql

BEGIN; -- Start a transaction

-- Step 1: Drop the old TEXT[] columns
ALTER TABLE public.listings DROP COLUMN IF EXISTS photos;
ALTER TABLE public.listings DROP COLUMN IF EXISTS tags;

-- Step 2: Rename the new JSONB columns to the original names
ALTER TABLE public.listings RENAME COLUMN photos_jsonb TO photos;
ALTER TABLE public.listings RENAME COLUMN tags_jsonb TO tags;

-- Step 3: Update View Definitions to use the renamed JSONB columns directly

-- View 1: listings_with_highest_bid
DROP VIEW IF EXISTS public.listings_with_highest_bid; -- Drop first to avoid type change issues
...
SELECT
    ...
    l.description,
    l.min_price,
    l.photos,  -- Now directly selects the renamed 'photos' (JSONB) column
    l.seller_id,
    l.end_time,
    l.created_at,
    l.upper_cap,
    l.rules,
    l.tags,    -- Now directly selects the renamed 'tags' (JSONB) column
    l.status,
    l.winning_bid_id,
    l.winning_bidder_id,
...
DROP VIEW IF EXISTS public.archived_listings_details; -- Drop first
...
SELECT l.id,
    ...
    l.upper_cap,
    l.rules,
    -- l.tags, -- This view did not have tags previously, uncomment if needed
    l.seller_id,
...
DROP VIEW IF EXISTS public.listings_with_seller_email; -- Drop first
...
-- Grant permissions again (CREATE OR REPLACE VIEW might reset them)
