...
export default function ListingDetails() {
    ...
    const load = async () => {
      try {
        const { data: lData, error: lError } = await supabase
          ...
          .select(
            ` id, title, description, min_price, photos, end_time, upper_cap, rules, seller_email, seller_id `
          )
          .eq('id', id)
          .maybeSingle();
            ...
            try {
              ...
              if (newBidDetails) {
                setBids((current) => [
                  ...
                  ...current.filter((b) => b.id !== newBidDetails.id),
                ]);
              }
            } catch (e) {
  ...
  const placeBid = async () => {
    ...
    // 2. Parse & validate whole-number bid
    // --- FIX: Use parseInt and check for integer ---
    const amt = parseInt(price, 10); // Parse as base-10 integer
    if (isNaN(amt) || !Number.isInteger(amt) || amt <= 0) {
      setBidStatusMessage(
        '⚠️ Please enter a valid whole number bid amount.'
      );
      return;
    }
    ...
    const minBidRequired = Math.max(
      listing.min_price,
      currentHighestBid
    );
    // --- Refined Check (isNaN redundant now) ---
    if (amt <= minBidRequired) {
      setBidStatusMessage(
        `⚠️ Bid must be greater than ${formatCurrency(minBidRequired)}`
      );
      return;
    }

    // 4. Upper-cap / Buy-Now checks (refined)
    if (listing.upper_cap && listing.upper_cap > 0) {
      if (amt > listing.upper_cap) {
        setBidStatusMessage(
          `⚠️ Bid cannot exceed the Buy Now price of ${formatCurrency(
            listing.upper_cap
          )}.`
        );
        return;
      ...
      if (amt === listing.upper_cap) {
        setBidStatusMessage(
          `⚠️ Bidding ${formatCurrency(
            listing.upper_cap
          )} triggers the Buy Now option (feature coming soon!).`
        );
        return;
      }
      // amt < upper_cap → allowed
    }
  ...
  const auctionEnded = listing.end_time
    ? isPast(listing.end_time)
    : false;
            ...
            {bidStatusMessage && (
              <p
                className={`mt-3 text-sm ${
                  bidStatusMessage.startsWith('✅')
                    ? 'text-green-600'
                    : bidStatusMessage.startsWith('⚠️')
                    ? 'text-yellow-700'
                    : 'text-red-600'
                }`}
              >
