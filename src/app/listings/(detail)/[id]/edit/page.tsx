...
export default function EditListingPage() {
  ...
  useEffect(() => {
    ...
    const fetchListingDetails = async () => {
      ...
      try {
        ...
        if (currentUser.id !== listingData.seller_id)
          throw new Error('N.A.E.');
        ...
        if (bidError)
          console.error('Failed to fetch bid count:', bidError.message);
        setHasBids(bidCount !== null && bidCount > 0);
      } catch (err: unknown) {
        let specificError =
          'An unexpected error occurred while loading the listing.';
        if (err instanceof Error) {
          if (err.message === 'F.L.D.')
            specificError = 'Failed to fetch listing details.';
          else if (err.message === 'L.N.F.')
            specificError = 'Listing not found.';
          else if (err.message === 'N.A.E.')
            specificError = 'You are not authorized to edit this listing.';
          else if (err.message.startsWith('L.S.N.A:'))
            specificError = `This listing is ${
              err.message.split(': ')[1]
    ...
    if (!hasBids) {
      ...
      if (upperCap.trim()) {
        ...
        if (isNaN(upperCapNum)) {
          ...
          messages.push(
            'Buy Now Price must be greater than Minimum Bid Price.'
          );
          isValid = false;
        }
      }
  ...
  const getStoragePathFromUrl = (url: string): string | null => {
    try {
      ...
      const objectPathIndex = pathSegments.findIndex(
  ...
  const processAndStageFiles = (files: FileList | null) => {
    ...
    if (filesArray.length > availableSlots) {
      showNotification(
        'error',
        `You can only upload ${MAX_PHOTOS} photos. You can add ${
    ...
    setPhotoPreviews((prev) => [
      ...prev,
      ...newDisplayPhotosToAdd.map((p) => p.url),
    ]);
  ...
  const handleDeletePhoto = (photoIdToDelete: string) => {
    ...
    if (photoToDeleteObj.isNew && photoToDeleteObj.file) {
      setNewPhotoFiles((prev) =>
        prev.filter((file) => file !== photoToDeleteObj.file)
      );
      URL.revokeObjectURL(photoToDeleteObj.url);
      setPhotoPreviews((prev) =>
        prev.filter((url) => url !== photoToDeleteObj.url)
      );
    } else {
  ...
  const handleSetAsPrimary = (photoIdToMakePrimary: string) => {
    setDisplayPhotos((prevPhotos) => {
      const itemIndex = prevPhotos.findIndex(
        (p) => p.id === photoIdToMakePrimary
      );
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const finalPhotoOrderForCheck = displayPhotos.filter(
      (p) => !(!p.isNew && photosToDelete.includes(p.url))
    );
    ...
    try {
      ...
      if (photosToDelete.length > 0) {
        ...
        if (pathsToDelete.length > 0) {
          ...
          if (deleteError)
            throw new Error(
              'Failed to remove some old photos. Please check your listing and try again.'
            );
        }
      }
      ...
      for (const displayPhoto of displayPhotos) {
        if (displayPhoto.isNew && displayPhoto.file) {
          ...
          const fileName = `${currentUser!.id}/${listingId}/${Date.now()}_${file.name.replace(
            /\s+/g,
            '_'
          )}`;
      ...
      const finalDatabasePhotoUrls = displayPhotos
        .map((dp) => {
          if (dp.isNew) {
            const uploaded = uploadedPhotoUrls.find(
              (up) => up.originalId === dp.id
            );
      ...
      >
          ...
          <div>
            ...
            {hasBids && (
              <p className="mt-1 text-xs text-gray-500 dark:text-bye-dark-text-secondary">
                Price cannot be changed after bids are placed.
              </p>
            )}
          </div>
        ...
        <div>
          <label className="block text-sm font-medium mb-1.5 text-gray-700 dark:text-bye-dark-text-primary">
            ...
            <span className="text-gray-500 dark:text-bye-dark-text-secondary text-xs ml-2">
              ({currentEffectivePhotoCount} / {MAX_PHOTOS} images)
            </span>
          </label>
          ...
          <div
            ...
            className={`mt-1 p-4 border-2 border-dashed rounded-md transition-colors ${
              isDraggingOver
                ? 'border-indigo-500 bg-indigo-50 dark:bg-bye-dark-bg-hover'
                : 'border-gray-300 dark:border-bye-dark-border-primary hover:border-gray-400 dark:hover:border-bye-dark-text-secondary'
            }`}
          >
            {/* preview grid */}
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 mb-4">
                {displayPhotos.map((photo, index) => (
                  ...
                  >
                    ...
                    {!photo.isNew &&
                      photosToDelete.includes(photo.url) && (
                        <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-60">
                          <p className="text-white text-[10px] sm:text-xs font-semibold text-center px-1">
                            Marked for Deletion
                          </p>
                        </div>
                      )}
            ...
            {currentEffectivePhotoCount < MAX_PHOTOS && (
              <label
                ...
                className="cursor-pointer flex flex-col items-center justify-center w-full py-6 text-center"
              >
                <svg
                  className="w-10 h-10 mb-3 text-gray-400 dark:text-bye-dark-text-secondary"
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 20 16"
                ...
                <p className="text-xs text-gray-500 dark:text-bye-dark-text-secondary">
                  PNG, JPG, WEBP (Max{' '}
                  {MAX_PHOTOS - currentEffectivePhotoCount > 0
                    ? MAX_PHOTOS - currentEffectivePhotoCount
                    : 0}{' '}
                  more)
                </p>
                <input
          ...
          <p className="mt-1.5 text-xs text-gray-500 dark:text-bye-dark-text-secondary">
            First image shown will be the cover photo. You can reorder by
            setting a new primary.
          </p>
        </div>
        ...
        <div className="pt-5 border-t border-gray-200 dark:border-bye-dark-border-primary">
          <button
            type="submit"
            disabled={
              isSubmitting || loadingPage || currentEffectivePhotoCount === 0
            }
