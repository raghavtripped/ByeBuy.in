...
export default function MyListingsPage() {
  ...
  useEffect(() => { /* ... same useEffect logic ... */
    const fetchUserDataAndListings = async () => {
        ...
        try {
            const { data: listingData, error: listingError } = await supabase.from('listings').select('id, title, description, min_price, end_time, created_at, photos').eq('seller_id', userData.user.id).order('created_at', { ascending: false });
  ...
  const handleDeleteListing = async (listingId: string, photoUrl: string | null) => { /* ... same handleDeleteListing logic ... */
    setDeletingId(listingId); setDeleteError(null);
    if (!window.confirm(/*...*/)) { setDeletingId(null); return; }
    try {
        const { error: bidsError, count: bidCount } = await supabase.from('bids').select('id', { count: 'exact', head: true }).eq('item_id', listingId);
